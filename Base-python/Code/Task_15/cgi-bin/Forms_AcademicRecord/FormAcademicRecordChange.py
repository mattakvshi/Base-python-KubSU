...
def changeData(con):
    ...
    if id != "-default-" and surnameListener != "-default-" and nameListener != "-default-" and patronymicListener != "-default-" and idSubject != "-default-" and mark != "-default-":
        temp_list = []
        ...
        temp_list.append(id)
        cursorObj.execute(
            "UPDATE AcademicRecord SET SurnameListener = ?, NameListener = ?, PatronymicListener = ?, IdSubject = ?, Mark = ? WHERE Id = ?",
            temp_list)
        message = f"<p>Запись с Id  '{id}' была изменена.</p>"
    elif id != "-default-" and surnameListener == "-default-" and nameListener == "-default-" and patronymicListener == "-default-" and idSubject == "-default-" and mark == "-default-":
        cursorObj.execute("DELETE FROM AcademicRecord WHERE Id = ?", (id,))
    ...
    <body>
        ...
        <div>
            ...
            <div class = "buttonWrapper">
                ...
                <a href = "http://localhost:8080/Forms_AcademicRecord/FormAcademicRecordChange.html"><button>Изменить данные ещё раз</button></a>
                <a href = "http://localhost:8080/Forms_AcademicRecord/FormAcademicRecordInsert.html"><button>Вставить данные</button></a>
            </div>
        </div>
    </body>
</html>"""
