...
def insertData(con):
    ...
    if first_name == "-default-" or last_name == "-default-" or birthday == "-default-" or user_age == "-default-" or registration_date == "-default-" or country == "-default-" or city == "-default-" or district == "-default-":
        message = "<p>Запись не была вставленна. Пожалуйста, введите все данные корректно.</p>"
    else:
        cursorObj.execute("INSERT INTO user(first_name, last_name,birthday, user_age, registration_date,country, city, district) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", temp_list)
        message = f"<p>Пользователь '{first_name, last_name,birthday, user_age, registration_date,country, city, district}' успешно добавлен в базу данных.</p>"
